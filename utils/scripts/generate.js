...
function generateCategorySectionHTML(cat, ifCategoryPage) {
    ...
    let categoryRssHtml = ''
    if (cat.rss && typeof(cat.rss) === "string" && ifCategoryPage) {
      categoryRssHtml = `<a href="${cat.rss}" target="_blank"><svg class="section-title-icon" alt="rss" title="rss" fill="currentColor" height="800px" width="800px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="-143 145 512 512" xml:space="preserve"><path d="M113,145c-141.4,0-256,114.6-256,256s114.6,256,256,256s256-114.6,256-256S254.4,145,113,145z M43.1,518.7 c-6.2,6.2-14.7,9.9-24.1,9.9c-9.4,0-17.8-3.8-24-9.9c-6.2-6.2-10-14.6-10-23.9c0-9.4,3.8-17.8,10-24s14.6-10,24-10 c9.4,0,17.9,3.8,24,10c6.2,6.2,10,14.6,10,24C53,504.2,49.2,512.6,43.1,518.7z M104.8,529c-0.1-32.1-12.5-62.3-35.1-84.9 c-22.6-22.6-52.8-35.2-84.7-35.2V360c46.6,0,88.7,19,119.3,49.6c30.6,30.6,49.5,72.8,49.6,119.4H104.8z M192,529 c-0.1-114.2-92.8-207.1-206.9-207.1V273c70.6,0,134.5,28.7,180.8,75.1c46.3,46.4,75,110.3,75.1,180.9H192z"/></svg></a>`
    }
    let pagePathHtml = ''
    if (cat.showPagePath !== false && cat.showPagePath !== "false") {
      pagePathHtml = `<h2 class="section-title ${ifCategoryPage ? 'section-title-category-page-path-nav' : ''}">${homeLink}/ <a href="${categoryPageName}">${categoryName}</a>${categoryRssHtml}</h2>`
    }
